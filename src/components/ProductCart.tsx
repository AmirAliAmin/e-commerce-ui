  ...
  return (
    <div className="shadow-lg rounded-lg overflow-hidden">
      ...
      <div className="relative aspect-2/3">
        <Image src={product.images[productType.color]} alt="product" fill className="object-cover hover:scale-105 hover:transition-all hover:duration-300"/>
      </div>
      </Link>
      {/* Product Detail */}
            ...
            <div className="flex items-center gap-2">
              {
                product.colors.map((color)=>(
                  <div className={ `cursor-pointer border rounded-full p-0.5 ${productType.color === color ? "border-gray-400" :"border-gray-200"}`} key={color} onClick={()=>handleProductType({type:"color", value:color})}>
